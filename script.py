...
#apx = 0.5
def default_evaluation_params(ap):
    """
...
if __name__ == '__main__':
    ...
    for ap in np.arange(0.5,1.0,0.05):
        evalParams = default_evaluation_params(ap)

        resDict = {'calculated': True, 'Message': '', 'method': '{}', 'per_sample': '{}'}
        evalData = evaluate_method(gtFilePath, submFilePath, evalParams)
        resDict.update(evalData)
        print("Calculated! for AP " + str(ap))
        print(resDict['method'])
        maps.append(resDict['method']['hmean'])
